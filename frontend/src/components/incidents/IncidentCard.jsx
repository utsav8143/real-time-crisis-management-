...
const IncidentCard = () => {
  ...
  useEffect(() => {
    async function fetchIncidents() {
      setIsLoading(true);
      try {
        const { data } = await api.get("/incident/view-incidents", {
  ...
  if (isLoading)
    return (
      <p className="p-4 text-white font-bold lg:text-lg">
        Loading Incidents...
      </p>
    );
  if (error)
    return (
      <p className="text-red-700 border border-red-700 rounded-xl p-2">
        {error}
      </p>
    );
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="overflow-y-auto  scrollbar scrollbar-thumb-gray-700 scrollbar-track-gray-900 scrollbar-thin overflow-hidden flex-1 min-h-0 ">
          ...
          {activeIncidents.map((incident) => {
            return (
              <div key={incident._id}>
                <div
                  className="border-b border-b-gray-700 grid grid-cols-4 p-4 "
                ...
                >
                  ...
                  >
                    <div
                      className=" "
                      style={{ color: SEV_COLOR[incident.severity] }}
                    >
                      {incident.severity}
                    </div>
                  </div>
                  ...
                  <div className="text-gray-500 lg:text-base text-sm ms-4">
